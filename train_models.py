...
df_students['attendance_code'] = le_attendance.fit_transform(df_students['attendance_status'])

le_assessment = LabelEncoder()
df_students['assessment_code'] = le_assessment.fit_transform(df_students['task_assessment_result'])

# Features & Target
X_stu = df_students[['attendance_code', 'assessment_code', 'time_on_task_minutes']]
y_stu = df_students['ai_training_risk_flag']

X_train_s, X_test_s, y_train_s, y_test_s = train_test_split(X_stu, y_stu, test_size=0.2, random_state=42)

model_students = RandomForestClassifier(n_estimators=100, random_state=42)
...
# Features & Target
X_veh = df_vehicles[['inventory_on_hand_qty', 'inventory_min_reorder_level', 'part_issued_qty', 'part_unit_cost']]
y_veh = df_vehicles['ai_reorder_status']

X_train_v, X_test_v, y_train_v, y_test_v = train_test_split(X_veh, y_veh, test_size=0.2, random_state=42)

model_inventory = RandomForestClassifier(n_estimators=100, random_state=42)
